...
$(this).css('max-height',h);}});}
    ...
    Webarch.prototype.initSideBar=function(){
        ...
        sidebar.find('li > a').on('mouseover',function(e){
            if($(this).parent().children('.sub-menu').length < 1){
                $(this).parent().parent().children('li.open').children('a').children('.arrow').removeClass('active');
                $(this).parent().parent().children('li.open').children('.sub-menu').slideUp(200);
                return;
            }
            var parent=$(this).parent().parent();//parent.children('li.open').children('a').children('.arrow').removeClass('open');
            var pli = $(this).parent();
            parent.children('li.open').each(
                function(obj, cb)
                {
                    //alert(pli == $(cb));
                    if(!$(cb).is(pli))
                    {
                        $(cb).children('a').children('.arrow').removeClass('active');
                        $(cb).children('.sub-menu').hide();
                    }
                }
            );
            ...
            if(sub.is(":visible")){
                ...
                sub.hide();//.slideUp(200,function(){});
            }
            else{
                jQuery('.arrow',jQuery(this)).addClass("open");
                jQuery(this).parent().addClass("open");
                sub.show();//.slideDown(200,function(){});
            }
            e.preventDefault();
        });
        if(sidebar.hasClass('mini')){
            ...
            elem.children('li.open').children('a').children('.arrow').removeClass('active');elem.children('li.open').children('.sub-menu').hide();elem.children('li').removeClass('open');
        }
...
Webarch.prototype.initSideBar=function(){
    var sidebar=$('.page-sidebar');
    var sidebarWrapper=$('.page-sidebar .page-sidebar-wrapper');
    sidebar.find('li > a').on('mouseover',
        function(e){
            if($(this).parent().children('.sub-menu').length < 1)
            {
                return;
            }//next().hasClass('sub-menu')===false){}
            var parent=$(this).parent().parent();
//parent.children('li.open').children('a').children('.arrow').removeClass('open');
            parent.children('li.open').children('a').children('.arrow').removeClass('active');
            parent.children('li.open').children('.sub-menu').slideUp(200);
//parent.children('li').removeClass('open');
            var sub=$(this).parent().children('.sub-menu').first();
            if(sub.is(":visible")){
                jQuery('.arrow',jQuery(this)).removeClass("open");
                jQuery(this).parent().removeClass("active");
...
Webarch.prototype.initSideBarToggle=function(){
    ...
    var $this=this;$('[data-webarch="toggle-left-side"]').on('touchstart click',function(e){e.preventDefault();$this.toggleLeftSideBar(); });
    $('[data-webarch="toggle-right-side"]').on('touchstart click',function(e){e.preventDefault();$this.toggleRightSideBar();});
}
Webarch.prototype.toggleLeftSideBar=function(){
    var timer;if($('body').hasClass('open-menu-left')){$('body').removeClass('open-menu-left');timer=setTimeout(function(){$('.page-sidebar').removeClass('visible');},300);}
...
$('#my-task-list').popover({html:true,content:function(){return $('#notification-list').html();}});$('#user-options').click(function(){$('#my-task-list').popover('hide');});$('table th .checkall').on('click',function(){if($(this).is(':checked')){$(this).closest('table').find(':checkbox').attr('checked',true);$(this).closest('table').find('tr').addClass('row_selected');}else{$(this).closest('table').find(':checkbox').attr('checked',false);$(this).closest('table').find('tr').removeClass('row_selected');}});}
...
$.Webarch=new Webarch();$.Webarch.Constructor=Webarch;})(window.jQuery);
$(document).ready(function(){
    $("#menu-collapse").click(function(){
        if($('.page-sidebar').hasClass('mini')){
            $('.page-sidebar').removeClass('mini');
            $('.page-content').removeClass('condensed-layout');
            $('.footer-widget').show();}else{$('.page-sidebar').addClass('mini');
    ...
    $('#layout-condensed-toggle').click(function(){
        if($('#main-menu').attr('data-inner-menu')=='1'){
            ...
            if($('#main-menu').hasClass('mini'))
            {
                ...
                $('.footer-widget').show();}else{$('body').addClass('grey');
...
$(function(){'use strict';$.Webarch.init();});